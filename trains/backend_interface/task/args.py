...
class _Arguments(object):
    ...
    def add_argument(self, option_strings, type=None, default=None, help=None):
        ...
        name = (
            option_strings[0].strip("- \t")
            if isinstance(option_strings, list)
            else option_strings.strip("- \t")
        )
    ...
    @classmethod
    def _add_to_defaults(
        cls, a_parser, defaults, a_args=None, a_namespace=None, a_parsed_args=None
    ):
        actions = [
            a
        ...
        try:
            if isinstance(a_parsed_args, dict):
                ...
                if a_parsed_args:
                    ...
                    args_dict = call_original_argparser(
                        a_parser, args=a_args, namespace=a_namespace
                    ).__dict__
            defaults_ = {
                a.dest: args_dict.get(a.dest)
                if (args_dict.get(a.dest) is not None)
                else ""
                for a in actions
            }
        except Exception:
            ...
            defaults_ = {
                a.dest: a.default if a.default is not None else "" for a in actions
            }
    ...
    def copy_defaults_from_argparse(
        self, parser, args=None, namespace=None, parsed_args=None
    ):
        task_defaults = {}
        ...
        if parsed_args:
            for k, v in parsed_args.__dict__.items():
                if k not in task_defaults:
                    # do not change this comparison because isinstance(type(v),
                    # type(None)) === False
    ...
    def copy_to_parser(self, parser, parsed_args):
        ...
        task_arguments = dict(
            [
                (k, v)
                for k, v in self._task.get_parameters().items()
                if not k.startswith(self._prefix_tf_defines)
                and self._exclude_parser_args.get(k, True)
            ]
        )
            ...
            for parent_parser, current_action in matched_actions:
                if current_action and isinstance(current_action, _StoreConstAction):
                    ...
                    const_value = (
                        ...
                        if current_action.const is not None
                        else (
                            current_action.default
                            if current_action.default is not None
                            else True
                        )
                    )
                    ...
                    try:
                        ...
                        pass
                    if current_action.default is not None or const_value not in (
                        None,
                        "",
                    ):
                        arg_parser_argeuments[k] = const_value
                elif current_action and (
                    current_action.nargs in ("+", "*")
                    or isinstance(current_action.nargs, int)
                ):
                    try:
                    ...
                    # if we have an int, we should cast to float, because it is
                    # more generic
                    if var_type == int:
                        var_type = float
                    # do not change! because isinstance(var_type, type(None))
                    # === False
                    elif isinstance(None, var_type):
                        var_type = str
                    # now we should try and cast the value if we can
                    try:
                        v = var_type(v)
                        # cast back to int if it's the same value
                        if isinstance(current_action.default, int) and int(v) == v:
                    ...
                    try:
                        if (
                            current_action.default is None
                            and current_action.type != str
                            and not v
                        ):
                            ...
                            # we will keep the type as int, just like the
                            # original argparser
                ...
                try:
                    if current_action and isinstance(current_action, _SubParsersAction):
                        if v not in (None, "") or current_action.default not in (
                            None,
                            "",
                        ):
                        ...
                        if v not in (None, "") or current_action.default not in (
                            ...
                            current_action.default = v
                        current_action.required = False
                        # python2 doesn't support defaults for positional
                        # arguments, unless used with nargs=?
                        if PY2 and not current_action.nargs:
                            current_action.nargs = "?"
                    else:
                        # do not add parameters that do not exist in argparser,
                        # they might be the dict
                        pass
        ...
        # if we already have an instance of parsed args, we should update its
        # values
        if parsed_args:
    ...
    def copy_to_dict(self, dictionary, prefix=None):
        ...
        for k, v in dictionary.items():
                ...
                try:
                    ...
                    try:
                        ...
                        self._task.log.warning(
                            "Failed parsing task parameter %s=%s keeping default %s=%s"
            ...
            try:
                # do not change this comparison because isinstance(v_type,
                # type(None)) === False
                if isinstance(None, v_type):
