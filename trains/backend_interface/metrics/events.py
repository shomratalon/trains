...
class ScalarEvent(MetricsEventAdapter):
    ...
    def __init__(self, metric, variant, value, iter, **kwargs):
        ...
        super(ScalarEvent, self).__init__(
            metric=metric, variant=variant, iter=iter, **kwargs
        )
...
class VectorEvent(MetricsEventAdapter):
    ...
    def __init__(self, metric, variant, values, iter, **kwargs):
        ...
        super(VectorEvent, self).__init__(
            metric=metric, variant=variant, iter=iter, **kwargs
        )
...
class ImageEventNoUpload(MetricsEventAdapter):
    ...
    def get_api_event(self):
        return events.MetricsImageEvent(
            url=self._url, key=self._key, **self._get_base_dict()
        )
...
class UploadEvent(MetricsEventAdapter):
    ...
    def _replace_slash(part):
        ...
        # notice % will be converted to %25 when the link is quoted, so we
        # should not use it
        return (
            part.replace("\\", "/")
            .strip("/")
    ...
    def __init__(
        ...
        **kwargs
        ...
        if image_data is not None and (
            not hasattr(image_data, "shape") and not isinstance(image_data, six.BytesIO)
        ):
        ...
        if not self._filename:
            if file_history_size < 1:
                ...
                self._filename = "%s_%s_%08d" % (
                    metric,
                    variant,
                    self._count % file_history_size,
                )
        ...
        if filename_ext is None:
            filename_ext = (
                ...
                + ".".join(
                    pathlib2.Path(self._local_image_path).parts[-1].split(".")[1:]
                )
            )
        ...
        self._override_storage_key_prefix = kwargs.pop(
            "override_storage_key_prefix", None
        )
    ...
    def get_file_entry(self):
        ...
        if abs(self._count - last_count) > self._file_history_size:
            ...
            if image_data.dtype != np.uint8:
                if (
                    np.issubdtype(image_data.dtype, np.floating)
                    and image_data.max() <= 1.0
                ):
    ...
    def get_target_full_upload_uri(
        self, storage_uri, storage_key_prefix=None, quote_uri=True
    ):
        e_storage_uri = self._upload_uri or storage_uri
        # if we have an entry (with or without a stream), we'll generate the
        # URL and store it in the event
