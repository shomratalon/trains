...
from .reqs import is_std_or_local_lib, project_import_modules
    ...
    def extract_reqs(self, module_callback=None, entry_point_filename=None):
        ...
        if entry_point_filename and not os.path.isabs(entry_point_filename):
            entry_point_filename = (
                os.path.join(self._project_path, entry_point_filename)
                if os.path.isdir(self._project_path)
                else None
            )

        # check if the entry point script is self contained, i.e. does not use
        # the rest of the project
        if (
            entry_point_filename
            and os.path.isfile(entry_point_filename)
            and not self._local_mods
        ):
            modules, try_imports, local_mods = project_import_modules(
                entry_point_filename, self._ignores
            )
            if not local_mods:
                ...
                num_local_mod = len(
                    set(self._local_mods.keys()) - set(self._force_modules_reqs.keys())
                )
        ...
        for name in self._local_mods:
            if name in modules:
                ...
                if os.path.commonpath(
                    [os.path.realpath(self._project_path)]
                ) == os.path.commonpath(
                    [
                        os.path.realpath(self._project_path),
