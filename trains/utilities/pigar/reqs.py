...
def project_import_modules(project_path, ignores):
    ...
    if os.path.isfile(project_path):
        ...
        for dirpath, dirnames, files in os.walk(
            os.path.abspath(project_path), followlinks=True
        ):
            ...
            if any(True for prefix in ignore_absolute if dirpath.startswith(prefix)):
                continue
            # Hack detect if this is a virtual-env folder, if so add it to the
            # uignore list
        ...
        if isinstance(value, ast.Call):
            if hasattr(value.func, "id"):
                ...
                # `__import__` function.
                    ...
                    self._modules.add(
                        node.value.args[0].s, self._fpath, node.lineno + self._lineno
                    )
                ...
                if module is not None and getattr(module, "id", "") == "importlib":
                    ...
                    if arg_len > 0 and hasattr(args[0], "s"):
                        ...
                        if not name.startswith("."):
                            self._modules.add(
                                name, self._fpath, node.lineno + self._lineno
                            )
                        elif arg_len == 2 and hasattr(args[1], "s"):
                            self._modules.add(
                                args[1].s, self._fpath, node.lineno + self._lineno
                            )
...
def is_std_or_local_lib(name):
    ...
    if exist and mpath is not None:
        if (
            "site-packages" in mpath
            or "dist-packages" in mpath
            or "bin/" in mpath
            and mpath.endswith(".py")
        ):
            exist = False
        elif (
            (sys.prefix not in mpath)
            and (sys.base_exec_prefix not in mpath)
            and (sys.base_prefix not in mpath)
        ):
            exist = mpath
...
def _search_path(path):
    ...
    for file in os.listdir(path):
        ...
        if fnmatch.fnmatch(file, "*-info"):
            ...
            # pep610 support. add support for new pip>=20.1 git reference
            # feature
            ...
            if os.path.isfile(git_direct_json):
                ...
                try:
                    ...
                    git_url = "{vcs}+{url}@{commit}#egg={package}".format(
                        vcs=vcs_info["vcs_info"]["vcs"],
                        url=vcs_info["url"],
                        commit=vcs_info["vcs_info"]["commit_id"],
                        package=pkg_name,
                    )
        ...
        # Install from local and available in GitHub.
            ...
            # Egg info path.
            info_dir = [
                _file for _file in os.listdir(dev_dir) if _file.endswith("egg-info")
            ]
            if not info_dir:
                continue
            info_dir = info_dir[0]
