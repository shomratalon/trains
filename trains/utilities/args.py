...
class PatchArgumentParser:
    ...
    def add_subparsers(self, **kwargs):
        if "dest" not in kwargs:
            if kwargs.get("title"):
                ...
                kwargs["dest"] = (
                    "/subparser%d" % PatchArgumentParser._add_subparsers_counter
                )
        return PatchArgumentParser._original_add_subparsers(self, **kwargs)

    @staticmethod
    def parse_args(self, args=None, namespace=None):
        return PatchArgumentParser._patched_parse_args(
    ...
    def _patched_parse_args(original_parse_fn, self, args=None, namespace=None):
        # if we are running remotely, we always have a task id, so we better
        # patch the argparser as soon as possible.
        if not PatchArgumentParser._current_task:
            from ..config import running_remotely

            if running_remotely():
                # this will cause the current_task() to set
                # PatchArgumentParser._current_task
        ...
        if PatchArgumentParser._current_task:
            ...
            if PatchArgumentParser._calling_current_task:
                # if we are here and running remotely by now we should try to
                # parse the arguments
                if original_parse_fn:
                    PatchArgumentParser._last_parsed_args = original_parse_fn(
            ...
            if running_remotely() and original_parse_fn:
                ...
                # if we do we need to patch the args, because there is no
                # default subparser
                if PY2:
                    import itertools
                    ...
                    def _get_sub_parsers_defaults(subparser, prev=[]):
                        ...
                        sub_parsers_defaults = (
                            [[subparser]]
                            if hasattr(subparser, "default") and subparser.default
                            else []
                        )
        ...
        PatchArgumentParser._last_parsed_args = (
            {}
            if not original_parse_fn
            else original_parse_fn(self, args=args, namespace=namespace)
        )
        return PatchArgumentParser._last_parsed_args
...
def patch_argparse():
    ...
    if not sys.modules.get("argparse") or hasattr(
        sys.modules["argparse"].ArgumentParser, "_parse_args_patched"
    ):
        return
    # mark patched argparse
    sys.modules["argparse"].ArgumentParser._parse_args_patched = True
    # patch argparser
    PatchArgumentParser._original_parse_args = sys.modules[
        "argparse"
    ].ArgumentParser.parse_args
    PatchArgumentParser._original_parse_known_args = sys.modules[
        "argparse"
    ].ArgumentParser.parse_known_args
    PatchArgumentParser._original_add_subparsers = sys.modules[
        "argparse"
    ].ArgumentParser.add_subparsers
    sys.modules["argparse"].ArgumentParser.parse_args = PatchArgumentParser.parse_args
    sys.modules[
        "argparse"
    ].ArgumentParser.parse_known_args = PatchArgumentParser.parse_known_args
    sys.modules[
        "argparse"
    ].ArgumentParser.add_subparsers = PatchArgumentParser.add_subparsers


# Notice! we are patching argparser, sop we know if someone parsed
# arguments before connecting to task
patch_argparse()


def call_original_argparser(self, args=None, namespace=None):
    if PatchArgumentParser._original_parse_args:
        return PatchArgumentParser._original_parse_args(
            self, args=args, namespace=namespace
        )
...
def get_argparser_last_args():
    return (
        PatchArgumentParser._last_arg_parser,
        PatchArgumentParser._last_parsed_args[0]
        if isinstance(PatchArgumentParser._last_parsed_args, tuple)
        else PatchArgumentParser._last_parsed_args,
    )
