...
class LoggerRoot(object):
    ...
    def _make_stream_handler(cls, level=None, stream=sys.stdout, colored=False):
        ...
        # if colored, try to import colorama & coloredlogs (by default, not in
        # the requirements)
        if colored:
            try:
                import colorama
    ...
    def get_base_logger(cls, level=None, stream=sys.stdout, colored=False):
        ...
        if redirect_level is not None and stream not in (None, sys.stderr):
            ...
            # and redirect is set for ERROR, in which case we redirect from
            # CRITICAL)
            ...
            if level < redirect_level:
                # Not all levels were redirected, remaining should be sent to
                # requested stream
                handler = cls._make_stream_handler(level, stream, colored)
                handler.addFilter(
                    _LevelRangeFilter(min_level=level, max_level=redirect_level - 1)
                )
...
def get_logger(path=None, level=None, stream=None, colored=False):
    ...
    root_log = LoggerRoot.get_base_logger(
        level=default_level, stream=sys.stdout, colored=colored
    )
...
def add_time_rotating_file_handler(
    logger, log_dir, log_file_prefix, when="midnight", formatter=None
):
    """
