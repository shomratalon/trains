...
from trains.backend_config.defs import (
...
try:
    def_host = (
        ENV_HOST.get(default=config_obj.get("api.web_server"))
...
def get_user_input():
    ...
    while True:
        ...
        if user_input.strip() == "}" and brackets_counter == 0:  # For break line input
            ...
            user_input.strip().startswith("credentials") and len(input_list) == 1
        ):  # For one line input
            return os.linesep.join(input_list)
...
def main():
    default_config_file = (
        os.getenv(LOCAL_CONFIG_FILE_OVERRIDE_VAR) or LOCAL_CONFIG_FILES[0]
    )
    ...
    """.format(
        CONFIG_FILE=args.file, HOST=def_host,
    )
    ...
    if parsed_host.netloc.startswith("demoapp."):
        ...
        api_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("demoapp.", "demoapi.", 1)
            + parsed_host.path
        )
        ...
        files_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("demoapp.", "demofiles.", 1)
            + parsed_host.path
        )
        ...
        api_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("app.", "api.", 1)
            + parsed_host.path
        )
        web_host = parsed_host.scheme + "://" + parsed_host.netloc + parsed_host.path
        files_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("app.", "files.", 1)
            + parsed_host.path
        )
        ...
        web_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("demoapi.", "demoapp.", 1)
            + parsed_host.path
        )
        files_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("demoapi.", "demofiles.", 1)
            + parsed_host.path
        )
    elif parsed_host.netloc.startswith("api."):
        print(
            "{} is the api server, we need the web server. Replacing 'api.' with 'app.'".format(
                parsed_host.netloc
            )
        )
        api_host = parsed_host.scheme + "://" + parsed_host.netloc + parsed_host.path
        web_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("api.", "app.", 1)
            + parsed_host.path
        )
        files_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace("api.", "files.", 1)
            + parsed_host.path
        )
        ...
        web_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace(":8008", ":8080", 1)
            + parsed_host.path
        )
        files_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace(":8008", ":8081", 1)
            + parsed_host.path
        )
        ...
        api_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace(":8080", ":8008", 1)
            + parsed_host.path
        )
        web_host = parsed_host.scheme + "://" + parsed_host.netloc + parsed_host.path
        files_host = (
            parsed_host.scheme
            + "://"
            + parsed_host.netloc.replace(":8080", ":8081", 1)
            + parsed_host.path
        )
    else:
        api_host = ""
        ...
        if not parsed_host.port:
            ...
            if not replace_port or replace_port == "y" or replace_port == "yes":
                ...
                not replace_port
                or replace_port.lower() == "n"
                or replace_port.lower() == "no"
            ):
        ...
        if not api_host:
            api_host = (
                parsed_host.scheme + "://" + parsed_host.netloc + parsed_host.path
            )
...
def input_url(host_type, host=None):
    while True:
        ...
        if host and (
            not parse_input
            or parse_input.lower() == "yes"
            or parse_input.lower() == "y"
        ):
            break
...
def verify_url(parse_input):
    try:
        if not parse_input.startswith("http://") and not parse_input.startswith(
            "https://"
        ):
            # if we have a specific port, use http prefix, otherwise assume
            # https
            if ":" in parse_input:
        ...
        parsed_host = None
        print(
            "Could not parse url {}\nEnter your trains-server host: ".format(
                parse_input
            ),
            end="",
        )
