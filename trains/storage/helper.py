...
class _DownloadProgressReport(object):
    ...
    def __call__(self, chunk_size):
        ...
        if self._verbose or (last_part >= self._report_chunk_size):
            ...
            self._log.info(
                "Downloading: %.0fMB / %.2fMb @ %.2fMbs from %s"
...
class _Driver(object):
    ...
    def upload_object_via_stream(
        self, iterator, container, object_name, extra, **kwargs
    ):
        pass
    ...
    @abstractmethod
    def download_object(
        self, obj, local_path, overwrite_existing, delete_on_failure, callback, **kwargs
    ):
        pass
...
class StorageHelper(object):
    ...
    class _PathSubstitutionRule(object):
        ...
        def load_list_from_config(cls):
            ...
            for index, sub_config in enumerate(
                config.get(cls.path_substitution_config, list())
            ):
                rule = cls(
                ...
                if any(
                    prefix is None
                    for prefix in (rule.registered_prefix, rule.local_prefix)
                ):
                    log.warning(
                        "Illegal substitution rule configuration '{}[{}]': {}".format(
                            cls.path_substitution_config, index, asdict(rule),
                        )
                    )
    ...
    # collect all bucket credentials that aren't empty (ignore entries with an
    # empty key or secret)
    _s3_configurations = S3BucketConfigurations.from_config(config.get("aws.s3", {}))
    _gs_configurations = GSBucketConfigurations.from_config(
        config.get("google.storage", {})
    )
    _azure_configurations = AzureContainerConfigurations.from_config(
        config.get("azure.storage", {})
    )
    ...
    def get(cls, url, logger=None, **kwargs):
        ...
        # Handle URL substitution etc before locating the correct storage
        # driver
        ...
        try:
            instance = cls(
                base_url=base_url, url=url, logger=logger, canonize_url=False, **kwargs
            )
            ...
            log.error(
                "Failed creating storage object {} Reason: {}".format(
                    base_url or url, ex
                )
            )
            return None
        ...
        **kwargs
        ...
        if self._scheme == _AzureBlobServiceStorageDriver.scheme:
            ...
            if self._conf is None:
                raise StorageError(
                    "Missing Azure Blob Storage configuration for {}".format(url)
                )

            if not self._conf.account_name or not self._conf.account_key:
                raise StorageError(
                    "Missing account name or key for Azure Blob Storage access for {}".format(
                        base_url
                    )
                )
            ...
            self._container = self._driver.get_container(
                container_name=self._base_url, config=self._conf
            )
            ...
            # If the scheme is file, use only the path segment, If not, use the
            # entire URL
    ...
    def terminate_uploads(cls, force=True, timeout=2.0):
        if force:
            # since async uploaders are daemon threads, we can just return and
            # let them close by themselves
            return
        # signal all threads to terminate and give them a chance for 'timeout'
        # seconds (total, not per-thread)
        cls._terminate.set()
    ...
    def get_configuration(cls, bucket_config):
        return cls._s3_configurations.get_config_by_bucket(
            bucket_config.bucket, bucket_config.host
        )
    ...
    def add_configuration(cls, bucket_config, log=None, _test_config=True):
        ...
        # Get existing config anyway (we'll either try to use it or alert we're
        # replacing it
        ...
        if not use_existing:
            ...
            if existing:
                if log:
                    log.warning(
                        "Overriding existing configuration for %s/%s"
            ...
            if existing:
                if log:
                    log.info(
                        "Using existing credentials for bucket %s/%s"
                        % (bucket_config.host or "AWS", bucket_config.bucket)
                    )
                good_config = _Boto3Driver._test_bucket_config(
                    existing, log, raise_on_error=False
                )
            ...
            if not good_config:
                ...
                if log:
                    log.info(
                        "Using global credentials for bucket %s/%s"
                        % (bucket_config.host or "AWS", bucket_config.bucket)
                    )
                if _test_config:
                    _Boto3Driver._test_bucket_config(bucket_config, log)
    ...
    def add_path_substitution(
        ...
        if replace_windows_sep and replace_linux_sep:
            raise UsageError(
                "Only one of replace_windows_sep and replace_linux_sep may be set."
            )
    ...
    def upload(
        ...
        if async_enable:
            ...
            return StorageHelper._upload_pool.apply_async(
                self._do_async_upload, args=(data,)
            )
        else:
            return self._do_upload(
                src_path, dest_path, extra, cb, verbose=False, retries=retries
            )
        ...
        if prefix:
            ...
            try:
                res = self._driver.list_container_objects(
                    self._container, ex_prefix=prefix
                )
            except TypeError:
            ...
            return [
                obj.name for obj in self._driver.list_container_objects(self._container)
            ]
    ...
    def download_to_file(
        ...
        try:
            ...
            if not overwrite_existing and Path(local_path).is_file():
                self._log.warning(
                    "File {} already exists, no need to download, thread id = {}".format(
                        local_path, threading.current_thread().ident,
                    ),
                )
            ...
            temp_local_path = "{}_{}{}".format(
                local_path, time(), self._temp_download_suffix
            )
            ...
            try:
                if isinstance(self._driver, _HttpDriver) and obj:
                    total_size_mb = float(obj.headers.get("Content-Length", 0)) / (
                        1024 * 1024
                    )
                elif hasattr(obj, "size"):
                    size = obj.size
                    # Google storage has the option to reload the object to get
                    # the size
            ...
            # if driver supports download with call back, use it (it might be
            # faster)
            if hasattr(self._driver, "download_object"):
                # callback
                cb = _DownloadProgressReport(
                    total_size_mb, verbose, remote_path, chunk_size_mb, self._log
                )
                ...
                stream = self._driver.download_object_as_stream(
                    obj, chunk_size_mb * 1024 * 1024
                )
                if stream is None:
            ...
            if verbose or download_reported:
                self._log.info(
                    "Downloaded %.2f MB successfully from %s , saved to %s"
    ...
    def download_as_stream(self, remote_path, chunk_size=None):
        ...
        try:
            ...
            self._log.error(
                "Could not download file : %s, err:%s " % (remote_path, str(e))
            )
            return None
    ...
    def download_from_url(cls, remote_url, local_path, overwrite_existing=False):
        ...
        return helper.download_to_file(
            remote_url, local_path, overwrite_existing=overwrite_existing
        )
    ...
    def _resolve_base_url(cls, base_url):
        ...
        if parsed.scheme == _Boto3Driver.scheme:
            ...
            if not conf:
                raise StorageError(
                    "Can't find azure configuration for {}".format(base_url)
                )
    ...
    def conform_url(cls, folder_uri, base_url=None):
        ...
        if not folder_uri.startswith(_base_url):
            ...
            if _base_url == "file://":
                ...
                log.debug(
                    "Upload destination {} amended to {} for registration purposes".format(
                        prev_folder_uri, folder_uri
                    )
                )
            else:
                raise ValueError(
                    "folder_uri: {} does not start with base url: {}".format(
                        folder_uri, _base_url
                    )
                )
    ...
    def _upload_from_file(self, local_path, dest_path, extra=None):
        if not hasattr(self._driver, "upload_object"):
            with open(local_path, "rb") as stream:
                res = self.upload_from_stream(
                    stream=stream, dest_path=dest_path, extra=extra
                )
        else:
            object_name = self._normalize_object_name(dest_path)
            extra = extra.copy() if extra else {}
    ...
    def _do_upload(
        self, src_path, dest_path, extra=None, cb=None, verbose=False, retries=1
    ):
        object_name = self._normalize_object_name(dest_path)
        if cb:
            try:
                ...
                self._log.error(
                    "Calling upload callback when starting upload: %s" % str(e)
                )
        if verbose:
        ...
        for i in range(max(1, retries)):
            try:
                self._upload_from_file(
                    local_path=src_path, dest_path=dest_path, extra=extra
                )
                last_ex = None
                break
            except Exception as e:
    ...
    def _get_object(self, path):
        ...
        try:
            ...
            self.log.warning(
                "Storage helper problem for {}: {}".format(str(object_name), str(e))
            )
            return None
...
class _HttpDriver(_Driver):
    ...
    class _Container(object):
        ...
        def __init__(self, name, retries=5, **kwargs):
            ...
            self.session = get_http_session_with_retry(
                total=retries, connect=retries, read=retries, redirect=retries
            )
        ...
        def get_headers(self, url):
            ...
            if url == self._default_files_server_host or url.startswith(
                self._default_files_server_host + "/"
            ):
    ...
    def get_container(self, container_name, config=None, **kwargs):
        if container_name not in self._containers:
            self._containers[container_name] = self._Container(
                name=container_name, retries=self._retries, **kwargs
            )
        return self._containers[container_name]

    def upload_object_via_stream(
        self, iterator, container, object_name, extra=None, **kwargs
    ):
        url = object_name[: object_name.index("/")]
        url_path = object_name[len(url) + 1 :]
        full_url = container.name + url
        # when sending data in post, there is no connection timeout, just an
        # entire upload timeout
        ...
        res = container.session.post(
            full_url,
            files={url_path: iterator},
            timeout=timeout,
            headers=container.get_headers(full_url),
        )
        if res.status_code != requests.codes.ok:
            raise ValueError(
                "Failed uploading object %s (%d): %s"
                % (object_name, res.status_code, res.text)
            )
        return res
    ...
    def get_object(self, container_name, object_name, *args, **kwargs):
        ...
        res = container.session.get(
            url, timeout=self.timeout, headers=container.get_headers(url)
        )
        if res.status_code != requests.codes.ok:
            raise ValueError(
                "Failed getting object %s (%d): %s"
                % (object_name, res.status_code, res.text)
            )
        return res
    ...
    def download_object(
        ...
        **_
        ...
        if not overwrite_existing and p.is_file():
            log.warning(
                "failed saving after download: overwrite=False and file exists (%s)"
                % str(p)
            )
            return
        length = 0
...
class _Boto3Driver(_Driver):
    ...
    class _Container(object):
        ...
        def __init__(self, name, cfg):
            ...
            self.name = name[5:]
            endpoint = (
                (("https://" if cfg.secure else "http://") + cfg.host)
                if cfg.host
                else None
            )
    ...
    def _get_stream_download_pool(self):
        if self._stream_download_pool is None:
            self._stream_download_pool = ThreadPoolExecutor(
                max_workers=self._stream_download_pool_connections
            )
    ...
    def get_container(self, container_name, config=None, **kwargs):
        if container_name not in self._containers:
            self._containers[container_name] = self._Container(
                name=container_name, cfg=config
            )
        ...
        try:
            container.bucket.upload_fileobj(
                ...
                Config=boto3.s3.transfer.TransferConfig(
                    ...
                    max_concurrency=self._max_multipart_concurrency
                    if container.config.multipart
                    else 1,
    ...
    def _test_bucket_config(
        cls, conf, log, test_path="", raise_on_error=True, log_on_error=True
    ):
        try:
            import boto3
            from botocore.exceptions import ClientError
        ...
        try:
            ...
            fullname = (
                furl(conf.bucket)
                .add(path=test_path)
                .add(path="%s-upload_test" % cls.__module__)
            )
            ...
            if region and (
                (conf.region and region != conf.region)
                or (not conf.region and region != "us-east-1")
            ):
                msg = (
                    "incorrect region specified for bucket %s (detected region %s)"
                    % (conf.bucket, region)
                )
            else:
                return True
    ...
    def _get_bucket_region(cls, conf, log=None, report_info=False):
        ...
        def report(msg):
            if (
                log
                and conf.get_bucket_host() not in cls._bucket_location_failure_reported
            ):
                if report_info:
                    log.debug(msg)
                else:
        ...
        try:
            ...
            return boto_resource.meta.client.get_bucket_location(Bucket=conf.bucket)[
                "LocationConstraint"
            ]
...
class _GoogleCloudStorageDriver(_Driver):
    ...
    def test_upload(self, test_path, config, **_):
        bucket_url = str(
            furl(scheme=self.scheme, netloc=config.bucket, path=config.subdir)
        )
        ...
        return set(test_obj.test_iam_permissions(permissions_to_test)) == set(
            permissions_to_test
        )
...
class _AzureBlobServiceStorageDriver(_Driver):
    ...
    def list_container_objects(self, container, ex_prefix=None, **kwargs):
        return list(
            container.blob_service.list_blobs(
                container_name=container.name, prefix=ex_prefix
            )
        )
    ...
    def delete_object(self, object, **kwargs):
        ...
        container.blob_service.delete_blob(
            container.name, object.blob_name,
        )
    ...
    def get_object(self, container_name, object_name, *args, **kwargs):
        ...
        if not container:
            raise StorageError(
                "Container `{}` not found for object {}".format(
    ...
    def download_object_as_stream(self, obj, *_, **__):
        ...
        blob = container.blob_service.get_blob_to_bytes(
            container.name, container.blob_name,
        )
        return blob.content
...
class _FileStorageDriver(_Driver):
    ...
    Object = namedtuple(
        "Object", ["name", "size", "extra", "driver", "container", "hash", "meta_data"]
    )
    ...
    def _check_container_name(self, container_name):
        ...
        if "/" in container_name or "\\" in container_name:
            raise ValueError(
                'Container name "{}" cannot contain \\ or / '.format(container_name)
            )
    ...
    def _make_container(self, container_name):
        ...
        try:
            ...
            raise OSError(
                'Target path "{}" is not accessible or does not exist'.format(full_path)
            )
    ...
    def _make_object(self, container, object_name):
        ...
        full_path = os.path.realpath(
            os.path.join(
                self.base_path, container.name if container else ".", object_name
            )
        )
    ...
    def get_container_cdn_url(self, container, check=False):
        ...
        path = os.path.realpath(
            os.path.join(self.base_path, container.name if container else ".")
        )
    ...
    def get_object_cdn_url(self, obj):
        ...
        return os.path.realpath(
            os.path.join(self.base_path, obj.container.name, obj.name)
        )
    ...
    def download_object(
        ...
        **_
        ...
        if os.path.exists(file_path) and not overwrite_existing:
            raise ValueError(
                'File "{}" already exists, but overwrite_existing=False'.format(
                    file_path
                )
            )
    ...
    def upload_object(
        self, file_path, container, object_name, extra=None, verify_hash=True, **_
    ):
        """
