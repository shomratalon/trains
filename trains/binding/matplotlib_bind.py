...
class PatchedMatplotlib:
    ...
    def patch_matplotlib():
        ...
        try:
            ...
            sys.modules[
                "matplotlib"
            ].figure.Figure.savefig = PatchedMatplotlib.patched_savefig
            ...
            try:
                ...
                warnings.filterwarnings(
                    action="ignore", category=UserWarning, module="plotly"
                )
            except Exception:
                pass
        ...
        try:
            ...
            PatchedMatplotlib._plotly_renderer = (
                PatchedMatplotlib._matplotlylib.PlotlyRenderer()
            )
        except Exception:
            pass
    ...
    def update_current_task(task):
        ...
        if PatchedMatplotlib._global_image_counter_limit is None:
            ...
            PatchedMatplotlib._global_image_counter_limit = config.get(
                "metric.matplotlib_untitled_history_size", 100
            )
        ...
        # if matplotlib is not loaded yet, get a callback hook
            ...
            and "matplotlib.pylab" not in sys.modules
        ):
            PatchedMatplotlib._current_task = task
            PostImportHookPatching.add_on_import(
                "matplotlib.pyplot", PatchedMatplotlib.patch_matplotlib
            )
            PostImportHookPatching.add_on_import(
                "matplotlib.pylab", PatchedMatplotlib.patch_matplotlib
            )
    ...
    def patched_imshow(*args, **kw):
        ...
        try:
            ...
            if stored_figure:
                stored_figure._trains_is_imshow = (
                    1
                    if not hasattr(stored_figure, "_trains_is_imshow")
    ...
    def patched_show(*args, **kw):
        ...
        try:
            ...
            for figure in figures:
                # if this is a stale figure (just updated) we should send it,
                # the rest will not be stale
                if figure.canvas.figure.stale or (
                    hasattr(figure, "_trains_is_imshow") and figure._trains_is_imshow
                ):
        ...
        if (
            PatchedMatplotlib._current_task
            and sys.modules["matplotlib"].rcParams["backend"] == "agg"
        ):
    ...
    def _report_figure(
        ...
        try:
            ...
            if specific_fig is None:
                # store the figure object we just created (if it is not already
                # there)
                ...
                if hasattr(stored_figure, "_trains_is_imshow"):
                    # flag will be cleared when calling clf() (object will be
                    # replaced)
                    stored_figure._trains_is_imshow = max(
                        0, stored_figure._trains_is_imshow - 1
                    )
            ...
            if force_save_as_image:
                ...
                image_format = "svg"
                # protect with lock, so we support multiple threads using the
                # same renderer
                ...
                try:
                    ...
                    def our_mpl_to_plotly(fig):
                        if (
                            not PatchedMatplotlib._matplotlylib
                            or not PatchedMatplotlib._plotly_renderer
                        ):
                            return None
                        PatchedMatplotlib._matplotlylib.Exporter(
                            PatchedMatplotlib._plotly_renderer, close_mpl=False
                        ).run(fig)
                        x_ticks = list(
                            PatchedMatplotlib._plotly_renderer.current_mpl_ax.get_xticklabels()
                        )
                        if x_ticks:
                            try:
                                ...
                                try:
                                    PatchedMatplotlib._plotly_renderer.plotly_fig[
                                        "layout"
                                    ]["xaxis1"].update(
                                        {
                                            "ticktext": [t.get_text() for t in x_ticks],
                                            "tickvals": [
                                                t.get_position()[0] for t in x_ticks
                                            ],
                                        }
                                    )
                                except BaseException:
                                    pass
                        y_ticks = list(
                            PatchedMatplotlib._plotly_renderer.current_mpl_ax.get_yticklabels()
                        )
                        if y_ticks:
                            try:
                                # check if all values can be cast to float
                                ...
                                try:
                                    PatchedMatplotlib._plotly_renderer.plotly_fig[
                                    ...
                                    ]["yaxis1"].update(
                                        {
                                            ...
                                            "tickvals": [
                                                t.get_position()[1] for t in y_ticks
                                            ],
                                        }
                                    )
                                except BaseException:
            ...
            if reporter is not None:
                ...
                # remove borders and size, we should let the web take care of
                # that
                if plotly_fig:
                    last_iteration = PatchedMatplotlib._get_last_iteration()
                    if plot_title:
                        title = PatchedMatplotlib._enforce_unique_title_per_iteration(
                            plot_title, last_iteration
                        )
                    else:
                    ...
                    if force_save_as_image or not PatchedMatplotlib._support_image_plot:
                        ...
                        if plot_title:
                            ...
                            title = "untitled %d" % (
                                PatchedMatplotlib._global_image_counter
                        ...
                        if plot_title:
                            ...
                            PatchedMatplotlib._global_plot_counter += 1
                            title = "untitled %d" % (
                                PatchedMatplotlib._global_plot_counter
                                % PatchedMatplotlib._global_image_counter_limit
                            )
    ...
    def _enforce_unique_title_per_iteration(title, last_iteration):
        if last_iteration != PatchedMatplotlib._last_iteration_plot_titles[0]:
            ...
            while title in PatchedMatplotlib._last_iteration_plot_titles[1]:
                # we already used this title in this iteration, we should
                # change the title
    ...
    def _get_last_iteration():
        ...
        reported_titles = ResourceMonitor.get_logger_reported_titles(
            PatchedMatplotlib._current_task
        )
