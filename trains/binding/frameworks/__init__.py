...
class WeightsFileHandler(object):
    ...
    def restore_weights_file(model, filepath, framework, task):
        ...
        if not filepath:
            get_logger(TrainsFrameworkAdapter).debug(
                "Could not retrieve model file location, model is not logged"
            )
            return filepath
        ...
        try:
            ...
            if ref_model is not None and model != ref_model():
                # old id pop it - it was probably reused because the object is
                # dead
            ...
            try:
                ...
                try:
                    config_text = (
                        trains_in_model.config_text if trains_in_model else None
                    )
                except Exception:
                    config_text = None
            ...
            try:
                ...
                ref_model = None
            WeightsFileHandler._model_in_store_lookup[id(model)] = (
                trains_in_model,
                ref_model,
            )
            ...
            if False and running_remotely():
                ...
                if (
                    ...
                    # actual model weights loading will be done outside the
                    # try/exception block
        except Exception as ex:
    ...
    def create_output_model(
        model, saved_path, framework, task, singlefile=False, model_name=None
    ):
        if task is None:
            return saved_path
        ...
        try:
            ...
            (
                trains_out_model,
                ref_model,
            ) = WeightsFileHandler._model_out_store_lookup.get(id(model), (None, None))
            # notice ref_model() is not an error/typo this is a weakref object
            # call
            if ref_model is not None and model != ref_model():
                # old id pop it - it was probably reused because the object is
                # dead
            ...
            # check if we have output storage, and generate list of files to
            # upload
            if Path(saved_path).is_dir():
                ...
                files = [
                    str(f)
                    for f in Path(saved_path).parent.glob(
            ...
            if trains_out_model is None:
                ...
                # if there is an output_uri, then by definition we will not
                # overwrite previously stored models.
            ...
            if trains_out_model.upload_storage_uri:
                if len(files) > 1:
                    ...
                    # protect against someone deletes/renames the file before
                    # async upload finish is done
                    target_filename = Path(files[0]).name
                    # HACK: if pytorch-lightning is used, remove the temp
                    # '.part' file extension
                    if sys.modules.get(
                        "pytorch_lightning"
                    ) and target_filename.lower().endswith(".part"):
                        target_filename = target_filename[: -len(".part")]
                    fd, temp_file = mkstemp(
                        prefix=".trains.upload_model_", suffix=".tmp"
                    )
                    os.close(fd)
                ...
                trains_out_model.update_weights(
                    weights_filename=None, register_uri=saved_path
                )
        except Exception as ex:
            get_logger(TrainsFrameworkAdapter).debug(str(ex))
        finally:
