...
class Artifact(object):
    ...
    def __init__(self, artifact_api_object):
        ...
        self._metadata = (
            dict(artifact_api_object.display_data)
            if artifact_api_object.display_data
            else {}
        )
        self._preview = (
            artifact_api_object.type_data.preview
            if artifact_api_object.type_data
            else None
        )
        self._object = None
    ...
    def get_local_copy(self, extract_archive=True, raise_on_error=False):
        ...
        if raise_on_error and local_copy is None:
            raise ValueError(
                "Could not retrieve a local copy of artifact {}, failed downloading {}".format(
                    self.name, self.url
                )
            )
...
class Artifacts(object):
    ...
    def register_artifact(self, name, artifact, metadata=None, uniqueness_columns=True):
        ...
        """
        # currently we support pandas.DataFrame (which we will upload as
        # csv.gz)
        if name in self._artifacts_container:
            LoggerRoot.get_base_logger().info(
                'Register artifact, overwriting existing artifact "{}"'.format(name)
            )
        self._artifacts_container.add_hash_columns(
            name,
            list(
                artifact.columns if uniqueness_columns is True else uniqueness_columns
            ),
        )
    ...
    def upload_artifact(
        self, name, artifact_object=None, metadata=None, delete_after_upload=False
    ):
        ...
        if name in self._artifacts_container:
            raise ValueError(
                "Artifact by the name of {} is already registered, use register_artifact".format(
                    name
                )
            )
        ...
        if np and isinstance(artifact_object, np.ndarray):
            ...
            if create_zip_file:
                ...
                override_filename_in_uri = (
                    folder.parts[-1] + override_filename_ext_in_uri
                )
                fd, zip_file = mkstemp(
                        ...
                        for filename in sorted(files):
                            if filename.is_file():
                                ...
                                artifact_type_data.preview += "{} - {}\n".format(
                                    relative_file_name,
                                    humanfriendly.format_size(filename.stat().st_size),
                                )
                                zf.write(
                                    filename.as_posix(), arcname=relative_file_name
                                )
                ...
                artifact_type_data.content_type = mimetypes.guess_type(artifact_object)[
                    0
                ]
            ...
            raise ValueError(
                "Artifact type {} not supported".format(type(artifact_object))
            )
        ...
        artifact = tasks.Artifact(
            ...
            display_data=[(str(k), str(v)) for k, v in metadata.items()]
            if metadata
            else None,
        )
    ...
    def _upload_data_audit_artifacts(self, name):
        ...
        override_filename_ext_in_uri = self._save_format
        override_filename_in_uri = name
        fd, local_csv = mkstemp(
            prefix=quote(name, safe="") + ".", suffix=override_filename_ext_in_uri
        )
        os.close(fd)
        local_csv = Path(local_csv)
        pd_artifact.to_csv(
            local_csv.as_posix(), index=False, compression=self._compression
        )
    ...
    def _upload_local_file(
        ...
        """
        upload_uri = (
            self._task.output_uri
            or self._task.get_logger().get_default_upload_destination()
        )
