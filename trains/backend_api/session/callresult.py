...
class CallResult(object):
    ...
    def __init__(
        self, meta, response=None, response_data=None, request_cls=None, session=None
    ):
        ...
        if response_data is not None:
            ...
            try:
                ...
                raise TypeError(
                    "response should be an instance of {}".format(Response.__name__)
                )
        else:
    ...
    @classmethod
    def from_result(
        cls, res, request_cls=None, logger=None, service=None, action=None, session=None
    ):
        ...
        try:
            ...
            raise ValueError(
                "Failed parsing meta section in response payload (data=%s, error=%s)"
                % (data, ex)
            )
    ...
    def ready(self):
        ...
        if res.status_code != session._async_status_code:
            self.__async_result = CallResult.from_result(
                res=res, request_cls=self.request_cls, logger=session._logger
            )
            return True
    ...
    def result(self):
        ...
        if self.__async_result is None:
            raise ResultNotReadyError(
                self._format_msg("Timeout expired"), call_id=self.meta.id
            )
            ...
            if not self.ready():
                ...
                if verbose and session._logger:
                    progress = (
                        "waiting forever"
                        if timeout is False
                        else "%.1f/%.1f seconds remaining"
                        % (remaining, float(timeout or 0))
                    )
                    session._logger.info(
                        "Waiting for asynchronous call %s (%s)"
                        % (self.request_cls.__name__, progress)
                    )
                time.sleep(poll_interval)
                remaining -= poll_interval
                continue
            # We've got something (good or bad, we don't know), create a call
            # result and return
            return self.result()

        # Timeout expired, return the asynchronous call's result (we've got
