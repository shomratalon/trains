# TRAINS - example of multiple sub-processes interacting and reporting to
# a single master experiment
...
if __name__ == "__main__":
    ...
    task = Task.init(
        "examples", "Popen example", auto_connect_arg_parser={"counter": False}
    )

    # we can connect multiple dictionaries, each from different process, as
    # long as the keys have different names
    param = {
        "args_{}".format(args.num_workers): "some value {}".format(args.num_workers)
    }
    task.connect(param)
    ...
    # We could also launch all of them here, but that would have been to
    # simple for us J
    if counter > 0:
        cmd = [
            sys.executable,
