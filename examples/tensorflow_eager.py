...
class Discriminator(tf.keras.Model):
    ...
    def __init__(self, data_format):
        ...
        self.conv2 = layers.Conv2D(128, 5, data_format=data_format, activation=tf.tanh)
...
class Generator(tf.keras.Model):
    ...
    def __init__(self, data_format):
        ...
        # We are using 128 6x6 channels as input to the first deconvolution
        # layer
        if data_format == "channels_first":
...
def train_one_epoch(
    ...
    for (batch_index, images) in enumerate(dataset):
        ...
        with tf.contrib.summary.record_summaries_every_n_global_steps(
            ...
            generator_grad = gen_tape.gradient(generator_loss_val, generator.variables)
            discriminator_grad = disc_tape.gradient(
                discriminator_loss_val, discriminator.variables
            )
            ...
            if log_interval and batch_index > 0 and batch_index % log_interval == 0:
                print(
                    "Batch #%d\tAverage Generator Loss: %.6f\t"
                    "Average Discriminator Loss: %.6f"
                    % (
                        batch_index,
                        total_generator_loss / batch_index,
...
def main(_):
    ...
    dataset = (
        tf.data.Dataset.from_tensor_slices(data.train.images[:1280])
        .shuffle(60000)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            "Directory for storing input data (default "
    ...
    parser.add_argument(
        ...
        metavar="N",
        help=(
            "number of batches between logging and writing summaries " "(default: 100)"
        ),
    )
    parser.add_argument(
        "--output_dir",
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        help=(
            "Directory to save checkpoints in (once per epoch) (default "
