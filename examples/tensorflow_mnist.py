...
weights = {
    "h1": tf.Variable(
        tf.random_normal([parameters["n_input"], parameters["n_hidden_1"]])
    ),
    "h2": tf.Variable(
        tf.random_normal([parameters["n_hidden_1"], parameters["n_hidden_2"]])
    ),
    "out": tf.Variable(
        tf.random_normal([parameters["n_hidden_2"], parameters["n_classes"]])
    ),
}
biases = {
...
optimizer = tf.train.AdamOptimizer(learning_rate=parameters["learning_rate"]).minimize(
    cost
)
