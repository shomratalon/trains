...
# Connecting TRAINS
task = Task.init(
    project_name="examples", task_name="Random Hyper-Parameter Search Example"
)
...
# Notice we can be more imaginative and use task_id which will eliminate
# the need to use project name
template_task = Task.get_task(
    project_name="examples", task_name=params["experiment_template_name"]
)

for i in range(params["total_number_of_experiments"]):
    # clone the template task into a new write enabled task (where we can
    # change parameters)
